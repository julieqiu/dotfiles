...
Please generate tasks from the PRD using the instructions below.

If not explicitly told which PRD to use, generate a list of PRDs and ask the
user to select one under `/tasks` or create a new one using `create-prd.md`:

- Assume the PRD is stored under `/tasks` and has a filename starting with
  `prd-` (e.g., `prd-[name].md`)
- It should not already have a corresponding task list in `/tasks` (e.g.,
  `tasks-prd-[name].md`)
- **Always** ask the user to confirm the PRD file name before proceeding Make
  sure to provide options in number lists so I can respond easily (if multiple
  options).
